...
def unpack_response(format, data):
    ...
    try:
        ...
        raise ValueError("Unexpected data returned from machine. Expected length=%i, got=%i, error=%s" %
...
def unpack_response_with_string(format, data):
    ...
    if (len(data) < struct.calcsize(format) + 1):
        raise ValueError("Not enough data received from machine, expected=%i, got=%i" %
                                   (struct.calcsize(format) + 1, len(data)))

    #Check for a null terminator on the string
    elif (data[-1]) != 0:
        raise ValueError("Expected null terminated string.")
