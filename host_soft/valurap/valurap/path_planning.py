...
import numpy
from numpy import array, absolute, arange
from numpy.linalg import norm
from scipy.optimize import minimize

from . import gcode
from .emulate import ir, int_x, int_v, int_a, vtoa_k, xtov_k, xtoa_k, FakeApg, max_xa, max_ya, max_ea, \
    max_delta, spm, spme, acc_step, v_step, format_segments
...
def process_plato(path, slowdowns):
    ...
    stage_ok = (sc["slowdown"] > 0.995).all()
...
def build_segments(path, slowdowns):
    ...
    lps_dx = lps_in["x1"] - lps_in["x0"]
    lps_dy = lps_in["y1"] - lps_in["y0"]
    lps_in["src_part"] = lps_dx.combine(lps_dy, lambda x1, x2: norm([x1, x2])) / lp_path["l"]
    ...
    lps_dx = lps_main["x1"] - lps_main["x0"]
    lps_dy = lps_main["y1"] - lps_main["y0"]
    lps_dn = lps_dx.combine(lps_dy, lambda x1, x2: norm([x1, x2]))
    lps_main["dt"] = lps_dn / lp_speeds["plato"]
    lps_main["src_part"] = lps_dn / lp_path["l"]
    ...
    lps_dx = lps_out["x1"] - lps_out["x0"]
    lps_dy = lps_out["y1"] - lps_out["y0"]
    lps_dn = lps_dx.combine(lps_dy, lambda x1, x2: norm([x1, x2]))

    lps_out["src_idx"] = lps["src_idx"]
    lps_out["src_part"] = lps_dn / lp_path["l"]
