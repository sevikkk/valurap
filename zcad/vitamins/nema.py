...
class Nema17(Unit):
    ...
    def shapes(self, config=None):
        ...
        bottom_plate = unify(
                ...
                + box(5 + 3, 16, plates_thick).translate(body_size/2 - 3, -8, 0).fillet(fillet_size)
        )
        top_plate = (
                plate
        ...
        for i in range(4):
            y = (i - 1.5) * 2
            cable = cylinder(r=1, h = 5).rotateY(deg(90)).translate(body_size/2+5, y, plates_thick/2)
            if cables:
                cables += cable
            else:
                cables = cable
    ...
    def get_connector(self, params="origin", config=None):
        ...
        if param == "origin":
           ...
           return Connector(position=[x, y, z], direction=[0,0,1], top=[1,0,0])
            ...
            return Connector(position=[0, 0, self.length], direction=[0,0,1], top=[1,0,0])
        elif param == "bottom":
            return Connector(position=[0, 0, 0], direction=[0,0,-1], top=[1,0,0])
