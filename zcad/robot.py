...
class Robot(Assembly):
    def get_subunits(self, part=None):
        ...
        indicator_transform = indicator.calculate_transform(
            pose={"origin": body.get_connector("indicator", part)}
        )

        head = HeadAssembly()
        head_transform = head.calculate_transform(
            pose={"origin": body.get_connector("head", part)}
        )
